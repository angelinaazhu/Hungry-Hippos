...
void ball_colour_calibration(int idx, SFE_ISL29125& RGB_sensor);
...
Serial.begin(9600); // terminal baud rate
  ...
  Serial.println("Sensor 1 & 2 init successful.");
...
void ball_colour_calibration(int idx, SFE_ISL29125& RGB_sensor){
  ...
  while (millis() - start < BALL_CALIB_DURATION) {
    light_LED(); // keep lighting up LED in case of disconnection
    sample_scaled_RGB(RGB_sensor); // red/green/blueVals all have scaled 0-255 vals
...
void ball_colour_calibration_helper(SFE_ISL29125& RGB_sensor){
  for (int i = 0; i < NUM_COLORS; ++i) {
    ...
    ball_colour_calibration(i, RGB_sensor);

...
//  use by doing TCAsel(0) -> TCAsel(pin)
...
const unsigned long INTENSITY_CALIB_DURATION =
    60000UL;  // calibrate light intensity duration
...
const unsigned long VOTING_INTERVAL =
    50UL;  // how long to wait between each vote
...
void setup() {
  ...
  Wire.begin();
  Wire.setClock(800000L);  // match other sketch (fast I2C)
...
void light_LED() {
  ...
  strip.setPixelColor(0, strip.Color(255, 255, 255));  // LED0:
                                                       // R,G,B=255,255,255
  strip.setPixelColor(1, strip.Color(255, 255, 255));  // LED1:
                                                       // R,G,B=255,255,255
...
void ball_colour_calibration(int idx) {
  ...
  double avgR = double(sumR) / count;  // avg of R: totalR / num of
                                       // accumulations
  ...
  // no per-color print here any more; printing is done as a single array after
  // all colors
}
...
void ball_colour_calibration_helper() {
  for (int i = 0; i < NUM_COLORS; ++i) {
    if (i == 0) {
      Serial.println(
          "Ensure NO ball is visible. Press any key to begin NO BALL "
          "(environment) calibration");
    } else {
      Serial.print("Place ");
      Serial.print(colorNames[i]);
      Serial.println(
          " ball in front of sensor. Press any key to begin calibration");
    }
    while (!Serial.available());
    Serial.read();

...
//  return true = time up
bool mini_countdown(Adafruit_7segment& hex_timer1,
                    Adafruit_7segment& hex_timer2, unsigned long& currentMillis,
                    unsigned long& previousMillis, unsigned int& secondsUntilGo,
                    bool& start) {
  ...
  if (currentMillis - previousMillis >= 1000) {
    ...
    // PRINT ON hex_timer1
    hex_timer1.writeDigitNum(0, sec1);
    hex_timer1.writeDigitNum(1, sec1);
    hex_timer1.drawColon(false);
    hex_timer1.writeDigitNum(3, sec1);
    hex_timer1.writeDigitNum(4, sec1);
    hex_timer1.writeDisplay();

    hex_timer2.writeDigitNum(0, sec1);
    hex_timer2.writeDigitNum(1, sec1);
    hex_timer2.drawColon(false);
    hex_timer2.writeDigitNum(3, sec1);
    ...
    if (secondsUntilGo > 0) {
      ...
      hex_timer1.writeDigitAscii(0, 71);
      hex_timer1.writeDigitAscii(1, 79);
      hex_timer1.drawColon(false);
      hex_timer1.writeDigitAscii(3, 79);
      hex_timer1.writeDigitAscii(4, 79);
      hex_timer1.writeDisplay();

      hex_timer2.writeDigitAscii(0, 71);
      hex_timer2.writeDigitAscii(1, 79);
      hex_timer2.drawColon(false);
      hex_timer2.writeDigitAscii(3, 79);
      hex_timer2.writeDigitAscii(4, 79);
      hex_timer2.writeDisplay();
...
// return false = time not up
// return true = time up
bool countdown(Adafruit_7segment& hex_timer1, Adafruit_7segment& hex_timer2,
               unsigned long& currentMillis, unsigned long& previousMillis,
